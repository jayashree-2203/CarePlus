...
public class ReceptionistDTO {
	...
	public void bookSlot(String patient_id) {
	    ...
	    if (!temp.app_list.containsKey(date)) {
	    ...
	    } else if (temp.app_list.get(date).size() < temp.getEndTime()) {
	...
	public void viewDoctorAppointment()
	{
		...
		for(Map.Entry<String,Doctor> i:object.doctor.entrySet())
		{
			for(Entry<LocalDate, ArrayList<Appointment>> j:i.getValue().app_list.entrySet())
			{
	...
	public void viewAllDoctorAppointment()
	{
		...
		for(Entry<LocalDate, ArrayList<Appointment>> j:doctor_obj.app_list.entrySet())
		{
			System.out.println(j.toString());
			System.out.println("\n\n-------------------\n");
		}
